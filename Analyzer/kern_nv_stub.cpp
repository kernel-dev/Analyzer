...
// NVIDIA GPU device stub
OSDefineMetaClassAndStructors(NVStub, IOService);

IOService *NVStub::probe(IOService *provider, SInt32 *score)
{
    IOPCIDevice *pciDevice = OSDynamicCast(IOPCIDevice, provider);
    if ( pciDevice == NULL )
    {
        DBGLOG("nv_stub", "probe: pciDevice is NULL - aborting");
        return nullptr;
    }
    
    DBGLOG("nv_stub", "probe: obtaining pci device model");
    
    OSString *modelProp = OSDynamicCast(OSString, pciDevice->getProperty("model"));
    
    if ( modelProp == NULL ) DBGLOG("nv_stub", "probe: failed to obtain pci device model");
    else {
    ...
    DBGLOG("nv_stub", "probe: attempting to obtain device and vendor ids");
    ...
    DBGLOG("nv_stub", "probe: finalized probe, letting real driver take over");

    // Must mark as failed so that the original driver can take over.
    return nullptr;
} // NVStub::probe()

bool NVStub::start(IOService *provider)
{
    SYSLOG("nv_stub", "start: shouldn't be enabled - aborting");
    return false;
} // NVStub::start()
